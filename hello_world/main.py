...
def main():
    ...
    print(
        f"      Статус код: {response.status_code}, Редирект на: {response.headers.get('location')}\n"
    )
    ...
    if p_doc:
        print(
            f"      Найдено по запросу 'иванов' (в БД 'Иванов...'): {p_doc['full_name']}\n"
        )
    else:
